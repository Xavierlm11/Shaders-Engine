...
void OnGlfwMouseMoveEvent(GLFWwindow* window, double xpos, double ypos)
{
    ...
    if (app->input.mouseButtons[MouseButton::RIGHT] == ButtonState::BUTTON_PRESSED)
    {
        if (app->firstClick)
        {
            ...
            app->lastMouseRightClick = BUTTON_PRESS;
        }
        else app->lastMouseRightClick = BUTTON_PRESSED;
        app->input.mouseDelta.x = xpos - app->input.mousePos.x;
        app->input.mouseDelta.y = app->input.mousePos.y - ypos;
        app->input.mousePos.x = xpos;
        app->input.mousePos.y = ypos;

        app->cam.yaw += app->input.mouseDelta.x;
        app->cam.pitch += app->input.mouseDelta.y;

        //app->cam.pitch = glm::clamp(app->cam.pitch, -89.0f, 89.0f);;
        if (app->cam.pitch > 89.0f) app->cam.pitch = 89.0f;
        if (app->cam.pitch < -89.0f) app->cam.pitch = -89.0f;
        ...
        app->cam.front = glm::normalize(front);
    }
    else if (app->lastMouseRightClick == BUTTON_PRESSED && app->input.mouseButtons[MouseButton::RIGHT] != ButtonState::BUTTON_PRESSED)
    {
        glfwSetInputMode(window, GLFW_CURSOR, GLFW_CURSOR_NORMAL);
        app->firstClick = true;
        app->lastMouseRightClick == BUTTON_IDLE;
    }
}
